...
final class VideoProcessor: FileProcessor, @unchecked Sendable {
  ...
  let supportedTypes: [UTType] = {
    ...
    let extensions = ["mp4", "mov", "m4v", "avi", "mkv", "webm"]
  ...
  func process(
    ...
    totalFrames = Int64(duration.seconds * Double(fps))
  ...
  private static func determineOutputUTI(from outputURL: URL, operations: [Operation]) -> UTType {
    ...
    // Default to MP4 using known identifier
    return UTType("public.mpeg-4") ?? UTType(filenameExtension: "mp4") ?? .audio
  }
